...
스크립트 ID·배포 ID는 채워 두었다. 브라우저로 수동 배포할 때만 쓴다. 평소에는 `node scripts/deploy-collector.mjs --stage` 로 새 버전을 만든 뒤, 소유자가 편집기에서 배포 관리 → 연필 → 그 버전 → 배포만 누른다.

> 2026-09-24 주의: 이 웹 앱은 "배포한 사용자로 실행"이다. dev@bottlecorp.kr 로 4단계 배포를 누르면 운영 콜렉터가 dev 계정으로 실행되기 시작하고, dev 계정이 스크립트 권한을 승인하지 않았으면 운영 전체가 멈춘다. 그래서 4단계는 **소유자 계정일 때만** 진행한다. 아래 4-5 확인에서 실행 사용자가 로그인 계정과 다르면 배포를 누르지 않는다.
...
5. "다음 사용자 인증 정보로 실행"과 "액세스 권한이 있는 사용자" 설정은 **바꾸지 않는다.** 현재 표시된 값을 그대로 기록한다. 실행 사용자가 "나(dev@bottlecorp.kr)"처럼 dev 계정으로 표시되면 **배포를 누르지 말고** 취소한 뒤 멈추고 보고한다(운영 실행 계정이 dev 로 바뀐다). 3단계에서 저장한 코드는 소유자가 이 화면에서 새 버전으로 배포하면 된다.
